...
<div class="card">
    <h5 class="card-header">Adăugați utilizator</h5>
    <div class="card-body">
      <form method="post" action="{{route('users.store')}}">
        {{csrf_field()}}
        <div class="form-group">
          <label for="inputTitle" class="col-form-label">Nume</label>
        <input id="inputTitle" type="text" name="name" placeholder="Introduceți nume"  value="{{old('name')}}" class="form-control">
        @error('name')
        ...
        <div class="form-group">
          ...
          <input id="inputEmail" type="email" name="email" placeholder="Introduceți email"  value="{{old('email')}}" class="form-control">
          @error('email')
          <span class="text-danger">{{$message}}</span>
          @enderror
        </div>
