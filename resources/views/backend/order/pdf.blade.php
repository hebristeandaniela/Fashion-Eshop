...
<html>
<head>
  <title>Comanda @if($order)- {{$order->order_number}} @endif</title>
...
@if($order)
    ...
    <div class="invoice-right-top float-right" class="text-right">
      <h3>Factură #{{$order->order_number}}</h3>
    ...
    <table class="table table-bordered table-stripe">
      ...
      <td><span>{{number_format($cart->price, 2)}} RON</span></td>
      </tr>
    @endforeach
      </tbody>
      <tfoot>
      ...
      <th scope="col"> <span>{{number_format($order->sub_total, 2)}} RON</span></th>
      </tr>
    ...
    $shipping_charge = DB::table('shippings')->where('id', $order->shipping_id)->pluck('price');
      @endphp
      <th scope="col" class="text-right ">Transport:</th>
      <th><span>{{number_format($shipping_charge[0], 2)}} RON</span></th>
      </tr>
      <tr>
      <th scope="col" class="empty"></th>
    ...
    <h4>Vă mulțumim pentru cumpărăturile dvs.!!</h4>
    </div>
    <div class="authority float-right mt-5">
    <p>-----------------------------------</p>
    <h5>Semnătura autorității:</h5>
    </div>
    <div class="clearfix"></div>
@else
